...
class PacingRequiredError(Exception):
    def __init__(self, seconds: int) -> None:
        ...
        super().__init__(
            f"Source pacing requires waiting about {seconds} seconds before another request"
        )


def build_search_url(
    adapter: dict[str, Any], base_url: str, query: str, page: int
) -> str:
    search = adapter["search"]
    encoded_query = (
        urllib.parse.quote(query, safe="")
        if search.get("query_encoding", "percent") == "percent"
        else query
    )
...
def _field_value(
    row: list[dict[str, object]], rule: dict[str, Any], base_url: str
) -> str | int | None:
    raw: str | None = None
    if "link_href_contains" in rule:
        ...
        cell = next(
            (
                item
                for item in row
                if str(rule.get("cell_class_contains", "")) in str(item["class"])
            ),
            None,
        )
        if (
            cell is None
            and "fallback_index" in rule
            and len(row) > int(rule["fallback_index"])
        ):
...
def parse_size_bytes(size: str | None) -> int | None:
    if not size:
        return None
    match = re.search(
        r"([0-9]+(?:[.,][0-9]+)?)\s*([KMGT]?i?B)", size, flags=re.IGNORECASE
    )
    if not match:
        return None
    ...
    return (
        round(float(match.group(1).replace(",", ".")) * factors[unit])
        if unit in factors
        else None
    )
...
def parse_torrent_date(value: str) -> str | None:
    ...
    relative = re.fullmatch(
        r"(\d+)\s+(minute|hour|day|week|month|year)s?\s+ago", lowered
    )
    if relative:
    ...
    normalised = re.sub(
        r"(\d{1,2})(?:st|nd|rd|th)", r"\1", cleaned, flags=re.IGNORECASE
    )
...
def parse_results(
    html: str, base_url: str, adapter: dict[str, Any]
) -> list[ParsedResult]:
    ...
    for row in parser.rows:
        if not row or (
            required_href
            and not any(
                href and required_href in href
                for cell in row
            ...
            continue
        values = {
            name: _field_value(row, rule, base_url) for name, rule in fields.items()
        }
        if not isinstance(values.get("title"), str) or not isinstance(
            values.get("details_url"), str
        ):
            continue
        parsed.append(
            ParsedResult(
                ...
                seeders=(
                    values.get("seeders")
                    if isinstance(values.get("seeders"), int)
                    else None
                ),
                leechers=(
                    values.get("leechers")
                    if isinstance(values.get("leechers"), int)
                    else None
                ),
                size=(
                    values.get("size") if isinstance(values.get("size"), str) else None
                ),
                uploader=(
                    values.get("uploader")
                    if isinstance(values.get("uploader"), str)
                    else None
                ),
                torrent_created_at=(
                    values.get("torrent_created_at")
                    if isinstance(values.get("torrent_created_at"), str)
                    else None
                ),
                size_bytes=parse_size_bytes(
                    values.get("size") if isinstance(values.get("size"), str) else None
                ),
            )
        )
...
def has_next_page(html: str, current_page: int, adapter: dict[str, Any]) -> bool:
    matches = re.findall(
        str(adapter.get("pagination", {}).get("href_regex", "(?!)")), html
    )
...
def parse_magnet_link(html: str, adapter: dict[str, Any]) -> str | None:
    ...
    return (
        unescape(match.group(1) if match and match.lastindex else match.group(0))
        if match
        else None
    )
...
def fetch_magnet_now(result_id: int) -> str | None:
    ...
    with connect() as db:
        item = db.execute(
            """SELECT r.*, s.kind, s.min_delay_seconds, s.current_delay_seconds, s.successful_requests, s.next_allowed_at,
               ...
               FROM results r JOIN sources s ON s.id=r.source_id WHERE r.id=?""",
            (result_id,),
        ).fetchone()
    if not item:
    ...
    try:
        ...
        _record_direct_magnet_result(
            item, "failed", getattr(error, "code", None), None, error
        )
...
def _record_direct_magnet_result(
    item: sqlite3.Row,
    status: str,
    ...
    with connect() as db:
        if status == "succeeded":
            db.execute(
                "UPDATE results SET magnet_link=? WHERE id=?", (magnet, item["id"])
            )
...
def run_one_job() -> bool:
    ...
    with connect() as db:
        ...
        if job:
            db.execute(
                "UPDATE crawl_jobs SET status='running', attempt_count=attempt_count+1 WHERE id=?",
                (job["id"],),
            )
            task = job
            request_type = "search"
            url = build_search_url(
                get_adapter(job["kind"]),
                job["base_url"],
                job["query"],
                job["next_page"],
            )
            ...
            task = db.execute(
                """SELECT d.*, j.query, j.status AS job_status, j.page_complete,
                   ...
                   ORDER BY d.run_after, d.id LIMIT 1""",
                (now, now),
            ).fetchone()
            if not task:
                return False
            db.execute(
                "UPDATE detail_tasks SET status='running', attempt_count=attempt_count+1 WHERE id=?",
                (task["id"],),
            )
            request_type = "detail"
...
def _complete_search_request(
    job: sqlite3.Row, url: str, html: str, http_status: int
) -> None:
    ...
    with connect() as db:
        ...
        for item in results:
            already_saved = db.execute(
                "SELECT 1 FROM results WHERE source_id=? AND details_url=?",
                (job["source_id"], item.details_url),
            ).fetchone()
            ...
            db.execute(
                ...
                (
                    ...
                    item.leechers,
                    item.uploader,
                    item.torrent_created_at,
                ),
            )
        db.execute(
            "UPDATE sources SET next_allowed_at=? WHERE id=?",
            (next_allowed, job["source_id"]),
        )
        if more_pages:
            db.execute(
...
def _complete_detail_request(
    task: sqlite3.Row, url: str, html: str, http_status: int
) -> None:
    ...
    with connect() as db:
        ...
        next_allowed = iso(utc_now() + timedelta(seconds=wait_after))
        db.execute(
            "UPDATE results SET magnet_link=? WHERE id=?", (magnet, task["result_id"])
        )
        db.execute(
            "UPDATE detail_tasks SET status='complete', attempt_count=0, last_error=NULL WHERE id=? AND status='running'",
            (task["id"],),
        )
        db.execute(
            "UPDATE sources SET next_allowed_at=? WHERE id=?",
            (next_allowed, task["source_id"]),
        )
        _record_request(
            db,
            task,
...
def _reschedule_failure(
    item: sqlite3.Row, request_type: str, url: str, error: Exception
) -> None:
    ...
    with connect() as db:
        ...
        if request_type == "search":
            db.execute(
                "UPDATE crawl_jobs SET status=?, run_after=?, last_error=? WHERE id=? AND status='running'",
                (status, next_run, message, item["id"]),
            )
        else:
            db.execute(
                "UPDATE detail_tasks SET status=?, run_after=?, last_error=? WHERE id=? AND status='running'",
                (status, next_run, message, item["id"]),
            )
        db.execute(
            "UPDATE sources SET next_allowed_at=? WHERE id=?",
            (next_run, item["source_id"]),
        )
        _record_request(
            db,
            item,
...
def _adjust_source_delay(
    db: sqlite3.Connection, source: sqlite3.Row, succeeded: bool
) -> tuple[int, int]:
    ...
    db.execute(
        "UPDATE sources SET current_delay_seconds=?, successful_requests=? WHERE id=?",
        (
            updated,
            streak,
            source["source_id"] if "source_id" in source.keys() else source["id"],
        ),
    )
