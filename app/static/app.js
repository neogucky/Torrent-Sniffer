...
function showAuth(setup) {
    ...
    $('#auth-confirm-wrap').hidden = !setup;
    $('#auth-submit').textContent = setup ? 'Create account' : 'Log in';
...
async function refreshSources() {
    ...
    $('#edit-source').disabled = disabled;
    $('#delete-source').disabled = disabled;
    $('#queue-search').disabled = disabled;
}
...
function actionButton(label, action, job, className = '') {
    ...
    button.onclick = async () => {
        try {
            await api(`/api/jobs/${job.id}?action=${action}`, {method: 'PATCH'});
            await refreshAll();
        } catch (error) {
            status(error.message);
        }
...
function taskCard(job) {
    ...
    heading.textContent = `#${job.id} · ${job.query}`;
    ...
    if (job.state !== 'running') {
        ...
        remove.onclick = () => deleteTask(job);
        actions.append(remove);
    }
    const details = document.createElement('button');
    details.textContent = 'Details';
    details.onclick = () => showRequestDetails(job.id);
...
async function showRequestDetails(jobId) {
    try {
        ...
        for (const request of requests) {
            ...
            const change = request.wait_adjustment_seconds ? ` (${request.wait_adjustment_seconds > 0 ? '+' : ''}${request.wait_adjustment_seconds}s)` : '';
...
async function refreshSummary() {
    ...
    $('#total-results').textContent = data.total_results;
    $('#total-crawls').textContent = data.total_crawls;
    $('#active-crawls').textContent = data.active_crawls || 0;
}

async function refreshQbittorrent() {
    qbittorrent = await api('/api/qbittorrent');
}

async function refreshAll() {
    await Promise.all([refreshAdapters(), refreshSources(), refreshJobs(), refreshSummary(), refreshQbittorrent()]);
}
...
async function searchLocal() {
    const rawQuery = $('#local-query').value.trim();
    const list = $('#results');
    const ranges = {
    ...
    if ($('#size-filter').value === 'custom') {
        minGb = Number($('#min-size-gb').value || 0);
        const rawMax = $('#max-size-gb').value;
        maxGb = rawMax === '' ? null : Number(rawMax);
    }
    if (minGb < 0 || (maxGb !== null && (maxGb < 0 || maxGb < minGb))) {
        status('Enter a valid custom size range.');
        return;
    }
    const gib = 1024 ** 3;
    ...
    for (const result of results) {
        const node = $('#result-template').content.cloneNode(true);
        const link = node.querySelector('.title');
        link.href = result.details_url;
        ...
        const magnet = node.querySelector('.magnet');
        const fetchButton = node.querySelector('.fetch-magnet');
        const qbitAction = node.querySelector('.qbit-action');
        const locationSelect = qbitAction.querySelector('select');
        const showQbittorrent = () => {
            qbitAction.style.display = 'none';
            if (!qbittorrent.configured) return;
            qbitAction.hidden = false;
            qbitAction.style.display = 'flex';
            locationSelect.replaceChildren(...qbittorrent.locations.map(location => new Option(location.label, location.label)));
            qbitAction.querySelector('.confirm-qbit').onclick = async () => {
                const button = qbitAction.querySelector('.confirm-qbit');
                button.disabled = true;
                button.innerHTML = '<span class="spinner" aria-hidden="true"></span> Downloading…';
                try {
                    await api(`/api/results/${result.id}/qbittorrent`, {
                        method: 'POST',
                        body: JSON.stringify({location_label: locationSelect.value})
                    });
                    button.classList.remove('download-error');
                    button.classList.add('download-success');
                    button.textContent = '✓';
                    button.title = 'Download sent to qBittorrent';
                    button.setAttribute('aria-label', 'Download sent');
                    status(`Sent to qBittorrent (${locationSelect.value}).`);
                } catch (error) {
                    showDownloadError(button, error.message);
                    status(error.message);
                }
            };
        };
        const showMagnet = (magnetLink, button) => {
        ...
        if (qbittorrent.configured) fetchButton.remove();
            ...
            button.onclick = async () => {
                ...
                button.disabled = true;
                button.innerHTML = '<span class="spinner" aria-hidden="true"></span> Fetching…';
                try {
                    const data = await api(`/api/results/${result.id}/magnet`, {method: 'POST'});
                    if (data.magnet_link) {
                        showMagnet(data.magnet_link, button);
                        status('Magnet link fetched.');
                    } else {
                        button.textContent = 'No magnet link found';
                        status('The detail page did not contain a magnet link.');
                    }
                } catch (error) {
                    button.disabled = false;
        ...
        node.querySelector('.query').textContent = `Collected from “${result.remote_query}” · ${result.discovered_at}`;
        list.append(node);
    }
...
function updateFilterLabels() {
    const size = $('#size-filter');
    const seeders = $('#seeder-filter');
    if (size.value === 'all') $('#size-summary').textContent = 'Size';
    else if (size.value === 'custom') {
        const min = $('#min-size-gb').value || '0';
        const max = $('#max-size-gb').value;
...
function refreshFilteredResults() {
    if ($('#results').childElementCount) searchLocal().catch(error => status(error.message));
}

function openCustomSizeDialog() {
    $('#custom-size-error').textContent = '';
    $('#custom-size-dialog').showModal();
}
...
function openSourceDialog(source = null) {
    ...
    const adapterSelect = $('#source-kind');
    adapterSelect.replaceChildren(...adapters.map(adapter => new Option(adapter.label, adapter.id)));
    adapterSelect.value = source?.kind ?? adapters[0]?.id ?? '';
    $('#source-url').value = source?.base_url ?? '';
    $('#delay').value = source?.min_delay_seconds ?? 20;
    $('#add-adapter').hidden = !currentUser?.is_admin;
    $('#edit-adapter').hidden = !currentUser?.is_admin;
...
$('#delete-source').onclick = async () => {
    const source = sources.find(item => item.id === Number($('#source-select').value));
    if (!source || !confirm(`Delete ${sourceName(source)}? Tasks will be removed but found results stay.`)) return;
    try {
        await api(`/api/sources/${source.id}`, {method: 'DELETE'});
        await refreshAll();
        status('Source deleted; found results were retained.');
    } catch (error) {
        status(error.message);
    }
};
...
$('#source-form').onsubmit = async (event) => {
    ...
    const payload = {
        base_url: $('#source-url').value,
        kind: $('#source-kind').value,
        min_delay_seconds: Number($('#delay').value)
    };
    try {
        await api(editingSourceId ? `/api/sources/${editingSourceId}` : '/api/sources', {
...
$('#adapter-form').onsubmit = async (event) => {
    ...
    let definition;
    try {
        definition = JSON.parse($('#adapter-json').value);
    } catch {
        status('Adapter JSON is invalid.');
        return;
    }
    try {
        const saved = await api(editingAdapterId ? `/api/adapters/${editingAdapterId}` : '/api/adapters', {
            method: editingAdapterId ? 'PUT' : 'POST',
            body: JSON.stringify({definition})
        });
        await refreshAdapters();
        $('#source-kind').replaceChildren(...adapters.map(adapter => new Option(adapter.label, adapter.id)));
        $('#source-kind').value = saved.id;
        $('#adapter-dialog').close();
        status('Adapter saved.');
    } catch (error) {
        status(error.message);
    }
};
...
$('#local-query').addEventListener('keydown', event => {
    if (event.key === 'Enter') searchLocal().catch(error => status(error.message));
});
...
$('#size-filter').onchange = () => {
    if ($('#size-filter').value === 'custom') openCustomSizeDialog(); else {
        closeFilter($('#size-filter'));
        refreshFilteredResults();
    }
};
$('#seeder-filter').onchange = () => {
    closeFilter($('#seeder-filter'));
    refreshFilteredResults();
};
$('#sort-results').onchange = refreshFilteredResults;
$('#clear-size').onclick = () => {
    $('#size-filter').value = 'all';
    $('#min-size-gb').value = '';
    $('#max-size-gb').value = '';
    closeFilter($('#size-filter'));
    refreshFilteredResults();
};
$('#clear-seeders').onclick = () => {
    $('#seeder-filter').value = '0';
    closeFilter($('#seeder-filter'));
    refreshFilteredResults();
};
$('#close-custom-size').onclick = $('#cancel-custom-size').onclick = () => {
    if (!$('#min-size-gb').value && !$('#max-size-gb').value) $('#size-filter').value = 'all';
    $('#custom-size-dialog').close();
    updateFilterLabels();
};
$('#custom-size-form').onsubmit = (event) => {
    event.preventDefault();
    const min = Number($('#min-size-gb').value || 0);
    const maxRaw = $('#max-size-gb').value;
    const max = maxRaw === '' ? null : Number(maxRaw);
    if (min < 0 || (max !== null && (max < 0 || max < min))) {
        $('#custom-size-error').textContent = 'Enter a valid size range.';
        return;
    }
    $('#size-filter').value = 'custom';
    $('#custom-size-dialog').close();
    closeFilter($('#size-filter'));
    refreshFilteredResults();
};
...
$('#auth-form').onsubmit = async (event) => {
    ...
    if (needsSetup && password !== $('#auth-confirm').value) {
        $('#auth-error').textContent = 'Passwords do not match.';
        return;
    }
    try {
        const user = await api(needsSetup ? '/api/auth/setup' : '/api/auth/login', {
            method: 'POST',
...
function addQbittorrentLocation(location = {label: '', path: ''}) {
    ...
    row.className = 'qbit-location-row';
    const label = document.createElement('input');
    label.placeholder = 'Label';
    label.value = location.label;
    const path = document.createElement('input');
    path.placeholder = 'Path in qBittorrent container';
    ...
    $('#qbit-locations').append(row);
}

function renderQbittorrentLocations(locations) {
    $('#qbit-locations').replaceChildren();
    (locations.length ? locations : [{label: '', path: ''}]).forEach(addQbittorrentLocation);
}
...
async function loadUsers() {
    ...
    for (const user of users) {
        ...
        label.textContent = `${user.username} · ${user.group}`;
        const actions = document.createElement('div');
        actions.className = 'user-actions';
        const edit = document.createElement('button');
        edit.type = 'button';
        edit.textContent = 'Edit';
        edit.onclick = () => openUserDialog(user);
        const remove = document.createElement('button');
        remove.type = 'button';
        remove.textContent = 'Remove';
...
function openUserDialog(user = null) {
    ...
    $('#user-group').value = user?.group ?? 'user';
    $('#user-password').value = '';
    $('#user-password').required = !user;
    $('#user-password-hint').hidden = !user;
...
async function removeUser(user) {
    ...
    try {
        ...
        await loadUsers();
        $('#settings-error').textContent = 'User removed.';
    } catch (error) {
...
async function openQbittorrentDialog() {
    try {
        ...
        $('#qbit-url').value = qbittorrent.base_url || '';
        $('#qbit-api-key').value = '';
        $('#qbit-api-key').placeholder = qbittorrent.configured ? 'Leave blank to keep existing key' : 'Required';
        $('#remove-qbit').hidden = !qbittorrent.configured;
        $('#qbit-error').textContent = '';
        renderQbittorrentLocations(qbittorrent.locations || []);
        $('#qbit-dialog').showModal();
    } catch (error) {
        $('#settings-error').textContent = error.message;
    }
}
...
$('#open-settings').onclick = async () => {
    ...
    const qbitManagement = $('#qbit-management');
    management.hidden = !currentUser?.is_admin;
    qbitManagement.hidden = !currentUser?.is_admin;
    try {
        if (currentUser?.is_admin) {
            await loadUsers();
            await refreshQbittorrent();
            renderQbittorrentSummary();
        }
...
$('#password-form').onsubmit = async (event) => {
    ...
    if ($('#new-password').value !== $('#confirm-new-password').value) {
        $('#settings-error').textContent = 'New passwords do not match.';
        return;
    }
    try {
        await api('/api/auth/password', {
            method: 'POST',
            body: JSON.stringify({new_password: $('#new-password').value})
        });
        $('#password-form').reset();
        $('#settings-error').textContent = 'Password changed.';
    } catch (error) {
        $('#settings-error').textContent = error.message;
    }
};
...
$('#user-form').onsubmit = async (event) => {
    ...
    try {
        await api(editingUserId ? `/api/auth/users/${editingUserId}` : '/api/auth/users', {
            method: editingUserId ? 'PUT' : 'POST',
            body: JSON.stringify({...payload, ...(editingUserId ? {} : {password})})
        });
        $('#user-dialog').close();
        await loadUsers();
        $('#settings-error').textContent = editingUserId ? 'User updated.' : 'User created.';
    } catch (error) {
...
$('#qbit-form').onsubmit = async (event) => {
    event.preventDefault();
    const locations = [...document.querySelectorAll('#qbit-locations .qbit-location-row')].map(row => ({
        label: row.children[0].value,
        path: row.children[1].value
    }));
    try {
        await api('/api/qbittorrent', {
            method: 'PUT',
            body: JSON.stringify({base_url: $('#qbit-url').value, api_key: $('#qbit-api-key').value || null, locations})
        });
        await refreshQbittorrent();
        renderQbittorrentSummary();
        $('#qbit-dialog').close();
        $('#settings-success').textContent = 'qBittorrent integration saved.';
    } catch (error) {
        $('#qbit-error').textContent = error.message;
    }
};
$('#remove-qbit').onclick = async () => {
    if (!confirm('Disable the qBittorrent integration?')) return;
    try {
        await api('/api/qbittorrent', {method: 'DELETE'});
        await refreshQbittorrent();
        renderQbittorrentSummary();
        $('#qbit-dialog').close();
        $('#settings-error').textContent = 'qBittorrent integration disabled.';
    } catch (error) {
        $('#qbit-error').textContent = error.message;
    }
};
